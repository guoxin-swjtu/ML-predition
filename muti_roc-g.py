...
print(roc_auc1)
# tmp = scipy.signal.savgol_filter(roc_auc1, 1, 1)
# z = np.array([0,0], [fpr1,tpr1])
plt.plot(fpr1, tpr1, 'g', label='FR-BO-glm = %0.3f' % roc_auc1, linewidth=2)
y_smoothed = gaussian_filter1d(z, sigma=5)
plt.plot(y_smoothed, label='拟合曲线', color='r')
...
print(roc_auc2)
plt.plot(fpr2, tpr2, 'navy', label='FR-BO-RF = %0.3f' % roc_auc2, linewidth=2, linestyle='-.')
...
print(roc_auc3)
plt.plot(fpr3, tpr3, 'r', label='FR-BO-svm = %0.3f' % roc_auc3, linewidth=2)
